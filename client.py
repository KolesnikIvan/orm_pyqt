...
class ClientSender(Thread, metaclass=ClientVerifier):
    ...
    def print_history(self):
        ...
        with db_lock:
            if ask == 'IN':
                hist_list = self.db.get_messages_list(to_whom=self.account_name)
                for message in hist_list:
                    print(f'message from {message[0]} to {message[3]}:{message[2]}')
            elif ask == 'OUT':
                hist_list = self.db.get_messages_list(from_whom=self.account_name)
                for message in hist_list:
                    print(f'message from {message[0]} to {message[3]}:{message[2]}')
            else:
                hist_list = self.db.get_messages_list()
                for message in hist_list:
                    print(f'message from {message[0]} to {message[3]}:{message[2]}')
